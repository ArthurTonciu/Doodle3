...
public class PlatformMoveY : MonoBehaviour
{
    ...
    void start()
    {
     ...
     void Update()
    {
        if (transform.position.y > platformY.transform.position.y + 4f)
            moveRight = false;
        if (transform.position.y < platformY.transform.position.y + 4f)
            moveRight = true;
    ...
    public void OnCollisionExit2D(Collision2D collision)            // столкновения (ну только это когда оно заканчивается)
    {
        if (collision.collider.name == "DeadZone")                  // если платформа встретилась с объектом с именем DeadZone
        {
            ...
            float RandY = Random.Range(transform.position.y + 12f, transform.position.y + 12.5f); // и новую позицию по у
